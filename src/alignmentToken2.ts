const MSG_NO_REPEATED_TOKENS =
  'there were substrings that were present in all lines of the selection. For alignment to work, make sure each line has a repeated string';
...
export function addTokens(
  ...
  searchTerms.forEach(term => {
    if (lines.every(line => line.includes(term))) {
      tokenContainer.push(
        new AlignmentToken(term, calculateIndices(lines, term))
      );
    }
  });
}
